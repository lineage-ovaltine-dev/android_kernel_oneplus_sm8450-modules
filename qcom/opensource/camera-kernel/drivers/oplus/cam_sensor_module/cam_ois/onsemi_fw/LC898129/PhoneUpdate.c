...
//**************************
CODE_TBL_EXT CdTbl[] = { { 0x0007, CcUpdataCode129, UpDataCodeSize,
			   UpDataCodeCheckSum, (void *)0, 0, 0, 0 },
...
UINT_8 PmemUpdate129(UINT_8 dist, CODE_TBL_EXT *ptr)
{
	...
	if (dist != 0) {
		...
		SizeofCode = (UINT_16)ptr->FromCode[9] << 8 |
			     (UINT_16)ptr->FromCode[8];
...
UINT_8 FlashBlockErase(UINT_8 SelMat, UINT_32 SetAddress)
{
	...
	if (SelMat != USER_MAT && SelMat != INF_MAT0 && SelMat != INF_MAT1 &&
	    SelMat != INF_MAT2)
		return 10;
	...
	IOWrite32A(FLASHROM_FLA_ADR,
		   ((UINT_32)SelMat << 16) | (SetAddress & 0x00003C00));
...
UINT_8 FlashSingleRead(UINT_8 SelMat, UINT_32 UlAddress, UINT_32 *PulData)
{
	...
	IOWrite32A(FLASHROM_FLA_ADR,
		   ((UINT_32)SelMat << 16) | (UlAddress & 0x00003FFF));
...
//********************************************************************************
UINT_8 FlashMultiRead(UINT_8 SelMat, UINT_32 UlAddress, UINT_32 *PulData,
		      UINT_8 UcLength)
{
	UINT_8 i;
...
UINT_8 FlashPageWrite(UINT_8 SelMat, UINT_32 SetAddress, UINT_32 *PulData)
{
	...
	IOWrite32A(FLASHROM_FLA_ADR,
		   ((UINT_32)SelMat << 16) | (SetAddress & 0x00003FF0));
...
//********************************************************************************
UINT_8 FlashProgram129(UINT_8 ModuleVendor, UINT_8 ActVer,
		       struct cam_ois_ctrl_t *o_ctrl)
{
	...
	if (sizeFromDts <= 0) {
		CAM_ERR(CAM_OIS, "ptr->SizeFromCode <= 0 s:%d",
			of_property_count_u8_elems(of_node, "fw_data"));
		return 0;
	}
	...
	if (!vaddr) {
		CAM_ERR(CAM_OIS, "Failed in allocating fw_data_size: %u",
			ptr->SizeFromCode);
		return 0;
	}

	ptr->FromCode = (uint8_t *)(vaddr);
	rc = of_property_read_u8_array(of_node, "fw_data", ptr->FromCode,
				       ptr->SizeFromCode);
	if (rc < 0) {
		CAM_ERR(CAM_OIS, "Invalid fw_data params");
	}

	rc = of_property_read_u32(of_node, "check_sum_size",
				  &ptr->SizeFromCodeValid);
	if (rc < 0) {
		CAM_ERR(CAM_OIS, "Invalid check_sum_size params");
	}
	CAM_INFO(CAM_OIS, "check_sum_size params :0x%x",
		 ptr->SizeFromCodeValid);
	...
	do {
		if (ptr->Index == (((UINT_16)ModuleVendor << 8) + ActVer)) {
			if ((!RamRead32A(0x8000, &ois_version)) &&
			    (!RamRead32A(0x8004, &oem_version))) {
				fw_version =
					((UINT_32)ptr->FromCode[158]) << 24 |
					((UINT_32)ptr->FromCode[159]) << 16 |
					((UINT_32)ptr->FromCode[160]) << 8 |
					(UINT_32)ptr->FromCode[161];
				CAM_INFO(
					CAM_OIS,
					"ois_version:0x%x  fw_version:0x%x oem_version:0x%x",
					ois_version, fw_version, oem_version);
				if (ois_version >= fw_version &&
				    (oem_version == 0x000107ff ||
				     oem_version == 0x00010700)) {
					CAM_INFO(
						CAM_OIS,
						"ois_version >= fw_version no need to update");
					vfree(vaddr);
					vaddr = NULL;
					return 0;
				}
				...
				CAM_ERR(CAM_OIS,
					"read ois current version 0x8000 failed ");
			}
			break;
		} else {
			CAM_ERR(CAM_OIS, "ptr->Index:0x%x Vendor:0x%x",
				ptr->Index,
				(((UINT_16)ModuleVendor << 8) + ActVer));
		}
		ptr++;
...
//********************************************************************************
void GetGyroOffset(UINT_16 *GyroOffsetX, UINT_16 *GyroOffsetY,
		   UINT_16 *GyroOffsetZ)
{
...
//********************************************************************************
void GetAcclOffset(UINT_16 *AcclOffsetX, UINT_16 *AcclOffsetY,
		   UINT_16 *AcclOffsetZ)
{
	UINT_32 ReadValX, ReadValY, ReadValZ;
...
//********************************************************************************
void MeasureStart(INT_32 SlMeasureParameterNum, INT_32 SlMeasureParameterA,
...
UINT_32 MeasGyAcOffset(void)
{
	...
	for (i = 0; i < 3; i++) {
		MeasAddressSelection(i, &SlMeasureParameterA,
				     &SlMeasureParameterB);

		MeasureStart(SlMeasureParameterNum, SlMeasureParameterA,
			     SlMeasureParameterB);
		...
		CAM_ERR(CAM_OIS, "(%d) AOFT = %08x, %08xh ", i,
			(unsigned int)StMeasValueA.StUllnVal.UlHigVal,
			(unsigned int)StMeasValueA.StUllnVal.UlLowVal);
		CAM_ERR(CAM_OIS, "(%d) BOFT = %08x, %08xh ", i,
			(unsigned int)StMeasValueB.StUllnVal.UlHigVal,
			(unsigned int)StMeasValueB.StUllnVal.UlLowVal);
		SlMeasureAveValueA[i] =
			(INT_32)((INT_64)StMeasValueA.UllnValue /
				 SlMeasureParameterNum);
		SlMeasureAveValueB[i] =
			(INT_32)((INT_64)StMeasValueB.UllnValue /
				 SlMeasureParameterNum);
	...
	if ((SlMeasureAveValueB[1]) >= POSTURETH) {
		...
		//UlRsltSts |= EXE_AZADJ ;
		CAM_ERR(CAM_OIS,
			"SlMeasureAveValueB[1]:0x%x POSTURETH:0x%x UlRsltSts:0x%x EXE_AZADJ",
			SlMeasureAveValueB[1], POSTURETH, UlRsltSts);
	}

	CAM_ERR(CAM_OIS, "AZOFF = %08xh UlRsltSts:0x%x",
		(unsigned int)SlMeasureAZ, UlRsltSts);
...
UINT_8 WrGyAcOffsetData(UINT_8 UcMode)
{
	...
	if (UcMode) {
		...
		CAM_ERR(CAM_OIS,
			"WrGyAcOffsetData: UlReadGx:0x%x UlReadGy:0x%x",
			UlReadGx, UlReadGy);

		UlMAT0[0] &= ~(0x00000100);
		UlMAT0[5] = (UINT_32)((UlReadAx & 0xFFFF0000) |
				      ((UlReadGx >> 16) & 0x0000FFFF));
		UlMAT0[6] = (UINT_32)((UlReadAy & 0xFFFF0000) |
				      ((UlReadGy >> 16) & 0x0000FFFF));
		UlMAT0[7] = (UINT_32)((UlReadAz & 0xFFFF0000) |
				      ((UlReadGz >> 16) & 0x0000FFFF));
	} else {
		UlMAT0[0] |= 0x00000100;
		UlMAT0[5] = 0x00000000;
		UlMAT0[6] = 0x00000000;
	...
	CAM_ERR(CAM_OIS, "WrGyAcOffsetData: [RVAL]:[BVal]=[%04x]:[%04x]",
		UsCkVal, UsCkVal_Bk);
