    ...
    struct cam_isp_hw_event_info       *event_info)
{
    ...
    case CAM_IFE_PIX_PATH_RES_RDI_0:
    case CAM_IFE_PIX_PATH_RES_RDI_1:
    // Fall through intended for standard epoch handling below
    // But handle specific RDI_2 SOF case first if enabled and requested
    case CAM_IFE_PIX_PATH_RES_RDI_2:
#ifdef OPLUS_FEATURE_CAMERA_COMMON //lanhe todo:
        // Check if we need to handle this RDI_2 event as a special SOF event
        if (ctx->flags.use_rdi_sof && (CAM_IFE_PIX_PATH_RES_RDI_2 == event_info->res_id))
        {
            struct cam_isp_hw_sof_event_data       sof_done_event_data;
            // Note: ife_hw_irq_epoch_cb already holds ctx->common.event_cb
            // Re-assigning to ife_hw_irq_sof_cb is possible but redundant here.
            // We can directly use ife_hw_irq_epoch_cb.
            cam_hw_event_cb_func ife_hw_irq_sof_cb = ctx->common.event_cb; // Keep original var if preferred

            // sof_done_event_data.is_secondary_evt = false; // Not needed, structure doesn't have this field
            ...
            // Using specific res_id for hack as per original code
            sof_done_event_data.res_id = CAM_ISP_HW_VFE_IN_RDI0;

            // Call the callback with SOF event type
            ife_hw_irq_sof_cb(ctx->common.cb_priv,
                    CAM_ISP_HW_EVENT_SOF, (void *)&sof_done_event_data);
            ...
            CAM_DBG(CAM_ISP,
                    ...
                    event_info->res_id);
            // Return early as this specific event is now handled
            return 0;
        }
#endif // OPLUS_FEATURE_CAMERA_COMMON
        // If the above #ifdef block didn't execute or didn't return,
        // proceed with the standard epoch handling for RDI_2 and others below.
    case CAM_IFE_PIX_PATH_RES_RDI_3:
    case CAM_IFE_PIX_PATH_RES_RDI_4:
    case CAM_IFE_PIX_PATH_RES_PPP:
        // Standard epoch handling for IPP, RDI 0-4, PPP
        if (atomic_read(&ctx->overflow_pending))
            break; // Break out of switch if overflow pending
        ...
        CAM_DBG(CAM_ISP,
            ...
            event_info->res_id);
        break; // Break out of switch after handling epoch

    default:
        CAM_ERR_RATE_LIMIT(CAM_ISP, "Invalid res_id: %d", event_info->res_id);
        break; // Break out of switch for default case
    } // end switch
